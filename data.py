...
for attempt in range(max_retries):
    try:
        ...
        ds = MsDataset.load('TsundereHan/Medicine', subset_name='default', split='train')
        print("数据集加载成功！")
        break
...
with open('train.jsonl', 'w', encoding='utf-8') as f:
    #with open(...) as f:：Python 的上下文管理器，用于安全地打开/关闭文件。
    for item in train_data:
        #json.dump()将 Python 对象（如字典、列表）转换为 JSON 格式的字符串，并直接写入文件对象
